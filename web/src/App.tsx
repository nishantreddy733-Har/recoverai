...
export default function App() {
  ...
  const razorpayCases = cases.filter((item) => item.id.startsWith("rzp_"));
  const workflowSteps = [
    { title: "Connect", complete: Boolean(razorpay?.connection?.ok), text: "Verify that RecoverAI can securely reach Razorpay Test Mode." },
    { title: "Intake failure", complete: (razorpay?.acceptedEvents ?? 0) > 0, text: "Send a signed test failure to create a pending Razorpay recovery case." },
    { title: "Run recovery", complete: razorpayCases.some((item) => item.status !== "pending"), text: "Process that case to generate a decision, audit trail and simulated outcome." },
  ];
  ...
  return <main>
    ...
    <section className="workflow-guide" aria-label="RecoverAI workflow">
      <div className="workflow-heading"><div><span className="eyebrow">START HERE</span><h2>Three steps from failure to recovery</h2></div><p>Checkout tests the payment experience. Metrics change only after a recovery case is created and processed.</p></div>
      <ol>{workflowSteps.map((step, index) => <li className={step.complete ? "complete" : ""} key={step.title}><span>{step.complete ? "✓" : index + 1}</span><div><strong>{step.title}</strong><p>{step.text}</p></div><b>{step.complete ? "Complete" : "Next"}</b></li>)}</ol>
    </section>
